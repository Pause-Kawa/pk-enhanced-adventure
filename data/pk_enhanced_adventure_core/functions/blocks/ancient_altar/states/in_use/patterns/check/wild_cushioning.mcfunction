...
execute if score %pk_has_enough_levels pk.common.value matches 0 if entity @e[type=armor_stand,tag=pk_current_entity,predicate=pk_enhanced_adventure_core:gameplay/ancient_altar/accepted_tools/ancient_tablet_wild_cushioning,nbt=!{HandItems:[{tag:{pkBewitchments:{WildCushioning:1b}}}]},distance=..0.1] run function pk_enhanced_adventure_core:blocks/ancient_altar/states/in_use/patterns/prepare_output/not_enough_level_placeholder

# If the pattern is correct and the user has enough levels
execute if score %pk_has_enough_levels pk.common.value matches 1 if entity @e[type=armor_stand,tag=pk_current_entity,predicate=pk_enhanced_adventure_core:gameplay/ancient_altar/accepted_tools/ancient_tablet_wild_cushioning,nbt=!{HandItems:[{tag:{pkBewitchments:{WildCushioning:1b}}}]},distance=..0.1] run function pk_enhanced_adventure_core:blocks/ancient_altar/states/in_use/patterns/prepare_output/wild_cushioning
