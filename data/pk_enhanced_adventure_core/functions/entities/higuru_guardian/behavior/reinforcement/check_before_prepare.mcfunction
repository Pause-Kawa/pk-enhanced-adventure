...
# If no valid player has been found (or if a single player in phase 3) skip the process, else, prepare
scoreboard players set %pk_temp_should_skip pk.common.value 0
execute if score %pk_temp pk.enhanced_adventure.counter matches 0 run scoreboard players set %pk_temp_should_reset pk.common.value 1
