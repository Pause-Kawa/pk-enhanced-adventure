...
# Search amount of Wildling Skeleton being in the room
execute at @e[type=marker,tag=pk_fight_engine_boss_room,limit=1,distance=..50] positioned ~-40 ~-2 ~-40 store result score %pk_temp_2 pk.enhanced_adventure.counter if entity @e[type=skeleton,tag=pk_wildling_skeleton,dx=80,dy=15,dz=80]

# If no valid player has been found, if a single player in phase 3 or if the amount of Wildling Skeletons in the room >3, skip the process, else, prepare
...
execute if score %pk_temp_should_skip pk.common.value matches 1 run function pk_enhanced_adventure_core:entities/wildling_guardian/behavior/reinforcement/skip
execute if score %pk_temp_should_skip pk.common.value matches 0 run function pk_enhanced_adventure_core:entities/wildling_guardian/behavior/reinforcement/start_prepare